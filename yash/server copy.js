...
// Middleware
app.use(cors());
app.use(express.json({ limit: "2gb" })); // Increase JSON payload limit for large documents
app.use(express.urlencoded({ extended: true, limit: "2gb" })); // Increase URL encoded limit
...
// Cache configuration
const CACHE_TTL = 7200; // 2 hours for large documents (longer cache)
const LARGE_DOC_THRESHOLD = 50 * 1024 * 1024; // 50MB threshold for large doc handling

class DocumentProcessor {
  constructor() {
    this.textSplitter = new RecursiveCharacterTextSplitter({
      chunkSize: 1000, // Larger chunks for big documents
      chunkOverlap: 200, // More overlap for better context
    });
  }
  ...
  async downloadAndExtractFromUrl(url, requestId) {
    try {
      ...
      const response = await axios.get(url, {
        ...
        maxBodyLength: 2 * 1024 * 1024 * 1024, // 2GB limit
      });
      ...
      // Cache the extracted text (only if not too large to avoid Redis memory issues)
      try {
        if (text.length < LARGE_DOC_THRESHOLD) {
          ...
          logger.info("Skipping cache for large document", {
            url,
            textLength: text.length,
            requestId,
          });
  ...
  async processAndStoreDocument(documentSource, requestId, fileType = null) {
    try {
      ...
      // Only save first 100 chunks to avoid huge log files
      const chunksToLog = docs.slice(0, 100);
      const chunks = chunksToLog
      ...
      logger.info("Chunks written to file (sample)", {
        chunksFilePath,
        totalChunksCount: docs.length,
        loggedChunksCount: chunksToLog.length,
        requestId,
      });
      ...
      // Process embeddings in batches to avoid memory issues with large documents
      ...
      let vectorStore = await FaissStore.fromDocuments(batches[0], embeddings);
      ...
      for (let i = 1; i < batches.length; i++) {
        ...
        const batchVectorStore = await FaissStore.fromDocuments(
          batches[i],
          embeddings
        );
        await vectorStore.mergeFrom(batchVectorStore);
      }
  ...
  async answerQuestions(questions, storeId, requestId) {
    try {
      ...
      const answerPromises = questions.map(async (question, index) => {
        ...
        // Retrieve top 5 most relevant docs (reduced for large documents)
        const relevantDocs = await vectorStore.similaritySearch(question, 5);

        // Prepare context from relevant documents
        const context = relevantDocs.map((doc) => doc.pageContent).join("\n\n");

        // Optimized prompt for faster response with large documents
        const prompt = `You are an expert assistant. Use ONLY the following context to answer the question.

Context:
${context.substring(0, 4000)} // Limit context to 4000 chars for large docs

Question:
${question}

Instructions:
- Give a concise answer that includes all critical details from the context.
